 ...
 * concrete, MGR ships canonical Markdown bodies for the five Runtime
 * Layer sections that PSF §11 reserves — the Bundler injects them
 * into every build so authors never have to write them by hand.
 *
 * Scope. This module owns runtime behavior only. Per PRD-004 §18, the
 * UI, the State Schema, and the response envelope are non-goals of
 * the Runtime Specification — they live in `src/contracts/` and are
 * pluggable per game. The runtime-level invariants those contracts
 * must respect (UI is a view of state, response follows commit, state
 * is the single source of truth) are baked into the bodies below so
 * they hold no matter which contract a game package supplies.
 *
 * The bodies are prose specifications, not prompts targeting a
...
export const RUNTIME_SPEC_VERSION = "2.0";
...
detective, strategy, interactive fiction. Only the game layer and
the presentation contracts (UI Contract, State Contract, Output
Contract) change; runtime behavior does not.`;
...
as nonexistent. The State Contract chooses the schema and
serialization; the invariant that state is the single source of
truth holds regardless of which contract the game package supplies.
...
Output invariants.
The response envelope is chosen by the Output Contract, but three
rules hold regardless of which envelope a game supplies:
- The response never mutates state.
- The response is generated after commit, not before.
- Every response offers at least one available action so the player
  is never left without a next move.`;
